...
public class RequestProcessor {
	...
	public Mono<ResponseEntity<?>> post(FunctionWrapper wrapper, String body,
		...
		if (!(function instanceof RoutingFunction) && input != null) {
	...
	private Mono<ResponseEntity<?>> response(FunctionWrapper wrapper, Object body,
		...
		if (function instanceof FluxedConsumer || function instanceof FluxConsumer) {
			...
			Object actualTargetFunction = this.getTargetIfRouting(wrapper, function);
			if (((Object) function) instanceof RoutingFunction
					&& actualTargetFunction instanceof FluxedConsumer || actualTargetFunction instanceof FluxConsumer) {
				RoutingFunction rFunction = (RoutingFunction) function;
				rFunction.accept((Publisher<Message<?>>) flux);
//				((Mono<?>) function.apply(flux)).subscribe();
				logger.debug("Handled POST with consumer");
				responseEntityMono = Mono
						.just(ResponseEntity.status(HttpStatus.ACCEPTED).build());
